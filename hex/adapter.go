...
func Parse(input io.Reader) (hexstr yara.HexTokens, err error) {
	// defer func() {
	// 	if r := recover(); r != nil {
	// 		if yaraError, ok := r.(yara.Error); ok {
	// 			err = yaraError
	// 		} else {
	// 			panic(r)
	// 		}
	...
	ParsedHexString = yara.HexTokens{}
